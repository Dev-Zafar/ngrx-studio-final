import { Request, Response } from 'express'
import nodemailer from 'nodemailer'
import { Contact } from '../models'
import { z } from 'zod'

const contactSchema = z.object({
  name: z.string().min(2),
  email: z.string().email(),
  budget: z.string().min(1),
  services: z.array(z.string()).min(1),
  brief: z.string().min(20).max(500),
})

export async function submitContact(req: Request, res: Response): Promise<void> {
  try {
    const data = contactSchema.parse(req.body)
    const contact = await Contact.create(data)

    // Send email notification
    if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
      const transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_PASS },
      })

      await transporter.sendMail({
        from: process.env.EMAIL_USER,
        to: process.env.EMAIL_TO || process.env.EMAIL_USER,
        subject: `🚀 New Project Brief from ${data.name}`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #080810; color: #F8F8FF; padding: 32px; border-radius: 12px;">
            <h2 style="color: #7C3AED; margin-bottom: 24px;">New Brief — NGRX Studio</h2>
            <table style="width: 100%; border-collapse: collapse;">
              <tr><td style="padding: 8px 0; color: #A0A0B8; width: 120px;">Name</td><td style="padding: 8px 0;">${data.name}</td></tr>
              <tr><td style="padding: 8px 0; color: #A0A0B8;">Email</td><td style="padding: 8px 0;">${data.email}</td></tr>
              <tr><td style="padding: 8px 0; color: #A0A0B8;">Budget</td><td style="padding: 8px 0;">${data.budget}</td></tr>
              <tr><td style="padding: 8px 0; color: #A0A0B8;">Services</td><td style="padding: 8px 0;">${data.services.join(', ')}</td></tr>
            </table>
            <div style="margin-top: 24px; padding: 16px; background: #0F0F1A; border-radius: 8px; border-left: 3px solid #7C3AED;">
              <p style="color: #A0A0B8; margin: 0 0 8px 0; font-size: 12px; text-transform: uppercase; letter-spacing: 2px;">Brief</p>
              <p style="margin: 0; line-height: 1.6;">${data.brief}</p>
            </div>
          </div>
        `,
      })
    }

    res.status(201).json({ message: 'Brief received', id: contact._id })
  } catch (err) {
    if (err instanceof z.ZodError) {
      res.status(400).json({ error: err.errors })
      return
    }
    res.status(500).json({ error: 'Server error' })
  }
}

export async function getContacts(req: Request, res: Response): Promise<void> {
  try {
    const contacts = await Contact.find().sort({ createdAt: -1 })
    res.json(contacts)
  } catch {
    res.status(500).json({ error: 'Server error' })
  }
}

export async function updateContactStatus(req: Request, res: Response): Promise<void> {
  try {
    const { status } = req.body
    const contact = await Contact.findByIdAndUpdate(req.params.id, { status }, { new: true })
    if (!contact) { res.status(404).json({ error: 'Not found' }); return }
    res.json(contact)
  } catch {
    res.status(500).json({ error: 'Server error' })
  }
}
